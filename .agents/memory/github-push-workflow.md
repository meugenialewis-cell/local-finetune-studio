...
**Working recipe (used for the Mac-installer task, July 2026):**
...
**Consequence / reconciliation (IMPORTANT for next push):** local main and origin/main now diverge in SHA (identical trees): local gets platform commit X, remote has API commit R, both children of the same parent. A future plain `git push` will fail non-fast-forward. Fix without blocked commands:
1. Verify trees match: `git rev-parse main^{tree}` vs remote commit R's tree (via API).
